...
import { getDate, saveTraining } from '../api/training'
...
export default function VoiceTraining() {
  ...
  useEffect(() => {
    getDate().then(d => setTodayKey(d.date)).catch(() => {
      const n = new Date()
  ...
  useEffect(() => {
    if (fin) {
      if (todayKey) saveTraining(todayKey, { voice: true })
      setPopup(true)
    }
  }, [fin, todayKey])
        ...
        setTimeLeft(t => {
          if (t <= 1) {
            ...
            if (idx < EXERCISES.length - 1) {
              const n = idx + 1
              setElapsed(e => e + EXERCISES[idx].duration)
              setIdx(n)
              setRunning(true)
  ...
  function goTo(i) {
    clearTimer()
    setIdx(i)
    setTimeLeft(EXERCISES[i].duration)
    setRunning(false)
    setElapsed(EXERCISES.slice(0, i).reduce((s, x) => s + x.duration, 0))
    setFin(false)
  }
  ...
  function skip() {
    if (idx < EXERCISES.length - 1) {
      const n = idx + 1
      clearTimer()
      setElapsed(EXERCISES.slice(0, n).reduce((s, x) => s + x.duration, 0))
      setIdx(n)
      setTimeLeft(EXERCISES[n].duration)
    } else {
      clearTimer()
      setElapsed(TOTAL)
      setTimeLeft(0)
  ...
  const totalElapsed = elapsed + (EXERCISES[idx].duration - timeLeft)
  ...
  if (fin) return (
    <div className="vt-fin">
      ...
      <Popup show={popup} icon="🎙️" title="ГОЛОС ПРОКАЧАН!"
        desc={<>Тренировка голоса завершена —<br/>возвращайся на главную</>}
      ...
      <div className="vt-main">
        <div className="vt-left">
          ...
          <div className="vt-controls">
            ...
            <button className="vt-ctrl-btn vt-ctrl-primary" style={{ background: ex.color }} onClick={() => setRunning(r => !r)}>
              {running ? '⏸' : '▶'}
            </button>
        ...
        <div className="vt-right">
          <div className="vt-instructions">
            <div className="vt-inst-header">
              <Icon type={ex.icon} size={20} color={ex.color}/>
              <span className="vt-inst-title">{ex.subtitle}</span>
            </div>
            <ul className="vt-inst-list">
            ...
            return (
              <button key={x.id} className={'vt-ex-item' + (active ? ' active' : '')} onClick={() => goTo(i)}>
                <div className={'vt-ex-icon' + (done ? ' done-icon' : '') + (active ? '' : ' dim')}>
                  {done
                    ? <svg width="20" height="20" viewBox="0 0 20 20" fill="none" stroke="#4ade80" strokeWidth="2.5" strokeLinecap="square"><polyline points="4,10 8,15 16,5"/></svg>
                    : <Icon type={x.icon} size={20} color={active ? x.color : '#45454F'}/>}
                </div>
                <span className={'vt-ex-name' + (done ? ' done-name' : '') + (!active && !done ? ' dim' : '')}>
                  {x.title}
                </span>
                <span className={'vt-ex-time' + (!active && !done ? ' dim' : '')} style={active ? { color: ex.color } : {}}>
                  {fmt(active ? timeLeft : x.duration)}
                </span>
              </button>
            )
          })}
