...
import { getDate, getTraining, saveTraining } from '../api/training'
import './CSTraining.css'
...
export default function CSTraining() {
  ...
  function toggle(i) {
    ...
    if (isAllDone) setPopup(true)
    if (todayKey) {
      saveTraining(todayKey, { cs: isAllDone, cs_tasks: next })
    }
  }

  function reset() {
    setChecked(Array(TASKS.length).fill(false))
    if (todayKey) saveTraining(todayKey, { cs: false, cs_tasks: [] })
  }
