  ...
  if (!authed) return (
    <div className="adm-login-wrap">
      <form className="adm-login-box" onSubmit={handleLogin}>
        <div className="adm-login-title">ГОЛОС ADMIN</div>
  ...
  return (
    <div className="adm-page">
      <div className="adm-header">
        <span className="adm-title">ГОЛОС ADMIN</span>
      ...
      <div className="adm-section-title">Тренировки голоса ({rows.length} записей)</div>
      ...
      {loading ? <div className="adm-loading">Загрузка...</div> : (
        <div className="adm-table-wrap">
          <table className="adm-table">
            ...
            <tbody>
              {rows.length === 0 && (
                <tr><td colSpan={3} className="adm-empty">Нет данных</td></tr>
              )}
              {rows.map(row => (
                <tr key={row.date}>
                  <td className="adm-date">{row.date}</td>
                  <td><span className={row.done ? 'adm-yes' : 'adm-no'}>{row.done ? '✓ Завершено' : '—'}</span></td>
