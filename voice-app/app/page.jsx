...
const HUB_URL = process.env.NEXT_PUBLIC_HUB_URL || 'http://illkiri02.fvds.ru'
...
function VoiceTrainingContent() {
  ...
  useEffect(() => {
    if (fin) {
      if (dateKey) fetch('/api/training/'+dateKey, { method:'POST', headers:{'Content-Type':'application/json'}, body: JSON.stringify({done:true}) })
      setPopup(true)
    }
  }, [fin, dateKey])
  ...
  if (fin) return (
    <div className="vt-fin">
      ...
      <Popup show={popup} icon="🎙️" title="ГОЛОС ПРОКАЧАН!" desc={<>Тренировка голоса завершена —<br/>возвращайся к дню</>} backHref={backHref} onClose={() => setPopup(false)}/>
    </div>
  )
